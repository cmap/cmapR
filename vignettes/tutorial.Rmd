...
vorinostat_ds3 <- subset_gct(my_ds,
                             cid=which(my_ds@cdesc$pert_iname=="vemurafenib"))
identical(vorinostat_ds, vorinostat_ds3)
```
...
```{r}
# melt to long form
vorinostat_ds3_melted <- melt.gct(vorinostat_ds3)
```

```{r}
# plot the matrix values grouped by gene
library(ggplot2)
ggplot(vorinostat_ds3_melted, aes(x=pr_gene_symbol, y=value)) +
  geom_boxplot() +
