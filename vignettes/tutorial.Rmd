...
(my_ds_no_meta <- parse.gctx(ds_path, matrix_only = TRUE))
```
...
(my_ds_no_meta <- annotate.gct(my_ds_no_meta, col_meta, dim="col",
                               keyfield="id"))
```
...
```{r}
# melt to long form
vemurafenib_ds3_melted <- melt.gct(vemurafenib_ds3)
```
...
write.gct(my_ds, "my_ds")
write.gctx(my_ds, "my_ds")
...
write.gctx(my_ds, "my_ds_compressed", compression_level = 9)
```
